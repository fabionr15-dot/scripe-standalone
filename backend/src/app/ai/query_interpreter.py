...
class InterpretedQuery:
    ...
    company_size: str | None = None  # small, medium, large
...
class QueryInterpreter:
    ...
    def _rule_based_interpret(self, query: str) -> InterpretedQuery:
        ...
        if detected_countries:
            # If multiple countries, use first one as primary but store all
            result.country = detected_countries[0]
            # Store additional countries in regions for now (will be used for multi-country search)
            if len(detected_countries) > 1:
                result.regions.extend([f"country:{c}" for c in detected_countries[1:]])
        else:
...
Respond ONLY with valid JSON in this exact format:
{
    ...
    "company_size": "small/medium/large or null",
...
- keywords_exclude should include terms the user wants to avoid (e.g., "corso", "formazione" if they say "no corsi")
